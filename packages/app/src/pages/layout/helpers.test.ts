...
import { type Session } from "@nexode-ai/sdk/v2/client"
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("nexode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("nexode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("nexode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("nexode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("nexode://open-project")).toBeUndefined()
    expect(parseDeepLink("nexode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "nexode://open-project?directory=/a",
      "nexode://other?directory=/b",
      "nexode://open-project?directory=/c",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __NEXODE__: {
        deepLinks: ["nexode://open-project?directory=/a"],
      },
    } as unknown as Window & { __NEXODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["nexode://open-project?directory=/a"])
