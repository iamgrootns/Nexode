...
import { createNexode, type ToolPart } from "@nexode-ai/sdk"
...
console.log("🚀 Starting nexode server...")
const nexode = await createNexode({
  port: 0,
})
console.log("✅ Nexode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await nexode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new nexode session...")
    const { client, server } = nexode
    ...
    console.log("✅ Created nexode session:", createResult.data.id)
  ...
  console.log("📝 Sending to nexode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Nexode response:", JSON.stringify(result, null, 2))
