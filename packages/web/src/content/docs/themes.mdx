...
With Nexode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Nexode uses our own `nexode` theme.
...
Nexode comes with several built-in themes.
...
- Want Nexode to match their terminal's appearance
...
Nexode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/nexode/themes/*.json` or `$XDG_CONFIG_HOME/nexode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.nexode/themes/*.json`
4. **Current working directory** - Defined in `./.nexode/themes/*.json`
