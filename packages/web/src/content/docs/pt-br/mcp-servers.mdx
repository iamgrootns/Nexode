...
Você pode adicionar ferramentas externas ao nexode usando o _Model Context Protocol_, ou MCP. O nexode suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do nexode](https://nexode.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/nexode`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O nexode lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o nexode irá:
...
Se o servidor requer autenticação, o nexode solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `nexode mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o nexode armazenará os tokens de forma segura em `~/.local/share/nexode/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no nexode, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do nexode como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o nexode à sua conta do Sentry.
