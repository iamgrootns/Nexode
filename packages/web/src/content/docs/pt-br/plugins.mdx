---
title: Plugins
description: Escreva seus próprios plugins para estender o nexode.
---

Plugins permitem que você estenda o nexode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do nexode.
...
- `.nexode/plugins/` - Plugins em nível de projeto
- `~/.config/nexode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/nexode/node_modules/`.
...
1. Configuração global (`~/.config/nexode/nexode.json`)
2. Configuração do projeto (`nexode.json`)
3. Diretório de plugins global (`~/.config/nexode/plugins/`)
4. Diretório de plugins do projeto (`.nexode/plugins/`)
...
O nexode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do nexode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o nexode.
...
:::note
Se você estiver usando o aplicativo desktop nexode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o nexode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao nexode:
...
O helper `tool` cria uma ferramenta personalizada que o nexode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o nexode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://nexode.ai/docs/sdk) para detalhes.
