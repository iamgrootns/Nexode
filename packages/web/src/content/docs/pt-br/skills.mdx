...
As habilidades do agente permitem que o nexode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O nexode pesquisa nesses locais:

- Configuração do projeto: `.nexode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/nexode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o nexode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.nexode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/nexode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.nexode/skills/git-release/SKILL.md` assim:
...
O nexode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `nexode.json`:
...
**Para agentes embutidos** (em `nexode.json`):
