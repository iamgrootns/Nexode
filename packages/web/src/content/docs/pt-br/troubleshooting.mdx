...
Para depurar problemas com o nexode, comece verificando os logs e os dados locais que ele armazena no disco.
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `nexode --log-level DEBUG`.

---

## Armazenamento

O nexode armazena dados de sessão e outros dados do aplicativo no disco em:
...
O nexode Desktop executa um servidor nexode local (o sidecar `nexode-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Nexode` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/nexode/nexode.jsonc` (ou `~/.config/nexode/nexode.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/nexode/nexode.jsonc`
...
O nexode também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o nexode possa reconstruí-lo.

1. Saia completamente do nexode Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/nexode`
- **Linux**: exclua `~/.cache/nexode` (ou execute `rm -rf ~/.cache/nexode`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\nexode`

3. Reinicie o nexode Desktop.

---

### Corrigindo problemas de conexão com o servidor

O nexode Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `nexode.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `NEXODE_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `NEXODE_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o nexode Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do nexode.

---

### Notificações não aparecendo

O nexode Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o nexode nas configurações do seu sistema operacional, e
...
1. Saia do nexode Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo nexode Desktop):

- `nexode.settings.dat` (URL do servidor padrão do desktop)
- `nexode.global.dat` e `nexode.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o nexode:
...
3. Certifique-se de que você tem a versão mais recente com `nexode upgrade`
...
Para descobrir quais modelos você tem acesso, execute `nexode models`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O nexode instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
2. Reinicie o nexode para reinstalar os pacotes de provedores mais recentes

Isso forçará o nexode a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O nexode detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
