...
Com o nexode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o nexode usa nosso próprio tema `nexode`.
...
O nexode vem com vários temas embutidos.
...
- Querem que o nexode corresponda à aparência do seu terminal
...
O nexode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/nexode/themes/*.json` ou `$XDG_CONFIG_HOME/nexode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.nexode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.nexode/themes/*.json`
