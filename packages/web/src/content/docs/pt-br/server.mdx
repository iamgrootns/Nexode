---
title: Servidor
description: Interaja com o servidor nexode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `nexode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente nexode pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `nexode.local` |
...
Defina `NEXODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `nexode`, ou defina `NEXODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `nexode serve` quanto ao `nexode web`.

```bash
NEXODE_SERVER_PASSWORD=your-password nexode serve
```
...
Quando você executa `nexode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor nexode para interagir com o nexode programaticamente.
:::

Essa arquitetura permite que o nexode suporte múltiplos clientes e permite que você interaja com o nexode programaticamente.

Você pode executar `nexode serve` para iniciar um servidor autônomo. Se você tiver o TUI do nexode em execução, `nexode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do nexode [IDE](/docs/ide).
...
O servidor nexode expõe as seguintes APIs.
