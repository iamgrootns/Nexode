...
To debug issues with Nexode, start by checking the logs and local data it stores on disk.
...
You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `nexode --log-level DEBUG`.

---

## Storage

nexode stores session data and other application data on disk at:
...
Nexode Desktop runs a local Nexode server (the `nexode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Nexode` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/nexode/nexode.jsonc` (or `~/.config/nexode/nexode.json`)
- **macOS/Linux** (older installs): `~/.local/share/nexode/nexode.jsonc`
...
Nexode can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Nexode can rebuild it.

1. Quit Nexode Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/nexode`
- **Linux**: delete `~/.cache/nexode` (or run `rm -rf ~/.cache/nexode`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\nexode`

3. Restart Nexode Desktop.

---

### Fix server connection issues

Nexode Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `nexode.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `NEXODE_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `NEXODE_PORT` (or pick a free port) and restart.
...
On Windows, Nexode Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with Nexode's features.

---

### Notifications not showing

Nexode Desktop only shows system notifications when:

- notifications are enabled for Nexode in your OS settings, and
...
1. Quit Nexode Desktop.
2. Find and delete these files (they live in the Nexode Desktop app data directory):

- `nexode.settings.dat` (desktop default server URL)
- `nexode.global.dat` and `nexode.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with Nexode:
...
3. Ensure you have the latest version with `nexode upgrade`
...
To figure out what models you have access to, run `nexode models`
...
If you encounter API call errors, this may be due to outdated provider packages. nexode dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart nexode to reinstall the latest provider packages

This will force nexode to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
nexode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
