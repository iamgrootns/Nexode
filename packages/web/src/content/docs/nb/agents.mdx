...
Det er to typer agenter i Nexode; primære agenter og underagenter.
...
Nexode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Nexode kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

Nexode kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `nexode.json`:
...
- Globalt: `~/.config/nexode/agents/`
- Per prosjekt: `.nexode/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Nexode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Nexode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Nexode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Nexode Zen](/docs/zen), vil du bruke `nexode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `nexode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/iamgrootns/Nexode).
:::
