---
title: Tjener
description: Samhandle med Nexode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `nexode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Nexode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `nexode.local` |
...
Sett `NEXODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `nexode` som standard, eller sett `NEXODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `nexode serve` og `nexode web`.
...
Når du kjører `nexode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Nexode-serveren til å samhandle med Nexode programmatisk.
:::

Denne arkitekturen lar Nexode støtte flere klienter og lar deg samhandle med Nexode programmatisk.

Du kan kjøre `nexode serve` for å starte en frittstående server. Hvis du har
Nexode TUI kjører, vil `nexode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Nexode [IDE](/docs/ide) plugins.
...
nexode-serveren viser følgende APIer.
