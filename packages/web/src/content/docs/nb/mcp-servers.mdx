...
Du kan legge til eksterne verktøy i Nexode ved å bruke _Model Context Protocol_ eller MCP. Nexode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Nexode Config](https://nexode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="nexode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/nexode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Nexode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Nexode:
...
Hvis serveren krever autentisering, vil Nexode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `nexode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Nexode lagre tokens sikkert i `~/.local/share/nexode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Nexode, sammen med innebygde verktøy. Så du kan administrere dem gjennom Nexode-konfigurasjonen som alle andre verktøy.
...
```json title="nexode.json" {11, 14-18}
{
  "$schema": "https://nexode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Nexode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Nexode-samtale oppført som en separat økt.
