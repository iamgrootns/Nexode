---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Nexode.
---

import config from "../../../../config.mjs"
export const console = config.console

Nexode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Nexode-konfigurasjonen.
...
i `~/.local/share/nexode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Nexode
konfig.
...
Nexode Zen er en liste over modeller levert av Nexode-teamet som har vært
testet og verifisert for å fungere godt med Nexode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Nexode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Nexode Zen` og gå til [nexode.ai/auth](https://nexode.ai/zen).
...
Det fungerer som alle andre leverandører i Nexode og er helt valgfritt å bruke.

---

## Nexode Go

Nexode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Nexode-teamet som har vært
testet og verifisert for å fungere godt med Nexode.

1. Kjør kommandoen `/connect` i TUI, velg `Nexode Go`, og gå til [nexode.ai/auth](https://nexode.ai/zen).
...
Slik bruker du Amazon Bedrock med Nexode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Nexode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `nexode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i nexode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Nexode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Nexode-konfigurasjonen.
...
for å lagre token i Nexode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Nexode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Nexode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`nexode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [nexode-gitlab-auth](https://www.npmjs.com/package/@gitlab/nexode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `nexode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Nexode:
...
Slik bruker du Google Vertex AI med Nexode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Nexode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Nexode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [nexode-helicone-session](https://github.com/H2Shami/nexode-helicone-session) plugin for automatisk å logge hver Nexode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Nexode-samtale oppført som en separat økt.
...
Du kan konfigurere Nexode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Nexode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Nexode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Nexode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/nexode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Nexode:
...
3. Kopier API-nøkkelen for bruk i Nexode.
...
6. **Viktig**: Før du bruker skymodeller i Nexode, må du hente modellinformasjonen lokalt:
...
Nexode Zen er en liste over testede og verifiserte modeller levert av Nexode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Nexode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Nexode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Nexode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Nexode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Nexode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Nexode:
...
Du kan også tilpasse modeller gjennom Nexode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Nexode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `nexode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Nexode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `nexode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Nexode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Nexode-konfigurasjonen.
