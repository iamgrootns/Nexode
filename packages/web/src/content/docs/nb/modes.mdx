...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i Nexode-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i Nexode lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom Nexode-konfigurasjonen.
...
Nexode kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.nexode/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `nexode.json` konfigurasjonsfil:
...
- Globalt: `~/.config/nexode/modes/`
- Prosjekt: `.nexode/modes/`
...
Hvis ingen temperatur er spesifisert, bruker Nexode modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Nexode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.nexode/modes/` for prosjektspesifikke moduser eller `~/.config/nexode/modes/` for globale moduser:
