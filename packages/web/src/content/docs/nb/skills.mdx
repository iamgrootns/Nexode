...
Agentferdigheter lar Nexode oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
nexode søker etter disse stedene:

- Prosjektkonfigurasjon: `.nexode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/nexode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går nexode opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.nexode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/nexode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.nexode/skills/git-release/SKILL.md` slik:
...
nexode viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `nexode.json`:
...
**For innebygde agenter** (i `nexode.json`):
