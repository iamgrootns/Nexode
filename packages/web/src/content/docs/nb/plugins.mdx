...
description: Skriv dine egne programtillegg for å utvide Nexode.
---

Plugins lar deg utvide Nexode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Nexode.
...
- `.nexode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/nexode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/nexode/nexode.json`)
2. Prosjektkonfigurasjon (`nexode.json`)
3. Global plugin-katalog (`~/.config/nexode/plugins/`)
4. Prosjektpluginkatalog (`.nexode/plugins/`)
...
Nexode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Nexode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Nexode.
...
:::note
Hvis du bruker Nexode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Nexode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Nexode:
...
`tool`-hjelperen lager et tilpasset verktøy som Nexode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Nexode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://nexode.ai/docs/sdk) for detaljer.
