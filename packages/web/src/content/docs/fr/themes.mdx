...
Avec Nexode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Nexode utilise notre propre thème `nexode`.
...
Nexode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Nexode corresponde à l'apparence de leur terminal
...
Nexode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/nexode/themes/*.json` ou `$XDG_CONFIG_HOME/nexode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.nexode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.nexode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/nexode/themes
vim ~/.config/nexode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .nexode/themes
vim .nexode/themes/my-theme.json
```
...
{
  "$schema": "https://nexode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
