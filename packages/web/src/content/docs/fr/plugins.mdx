---
title: Plugins
description: Écrivez vos propres plugins pour étendre Nexode.
---

Les plugins vous permettent d'étendre Nexode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Nexode.
...
- `.nexode/plugins/` - Plugins au niveau du projet
- `~/.config/nexode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/nexode/node_modules/`.
...
1. Configuration globale (`~/.config/nexode/nexode.json`)
2. Configuration du projet (`nexode.json`)
3. Répertoire global des plugins (`~/.config/nexode/plugins/`)
4. Répertoire des plugins du projet (`.nexode/plugins/`)
...
Nexode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK nexode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre nexode.
...
:::note
Si vous utilisez l'application de bureau Nexode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez nexode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à nexode :
...
L'assistant `tool` crée un outil personnalisé que nexode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour nexode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://nexode.ai/docs/sdk) pour plus de détails.
