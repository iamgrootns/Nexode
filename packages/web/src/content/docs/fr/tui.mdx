---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Nexode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Nexode fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Nexode démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Nexode TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Nexode. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `nexode.json`, qui configure le comportement du serveur/d'exécution.
...
Utilisez `NEXODE_TUI_CONFIG` pour charger un chemin de configuration TUI personnalisé.
