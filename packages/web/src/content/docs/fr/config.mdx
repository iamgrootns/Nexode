---
title: Configuration
description: Utilisation de la configuration JSON Nexode.
---

Vous pouvez configurer Nexode à l'aide d'un fichier de configuration JSON.

---

## Format

Nexode prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).

```jsonc title="nexode.jsonc"
{
...
1. **Configuration distante** (à partir de `.well-known/nexode`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/nexode/nexode.json`) - préférences utilisateur
3. **Configuration personnalisée** (`NEXODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`nexode.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.nexode`** - agents, commandes, plugins
6. **Configuration en ligne** (`NEXODE_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.nexode` et `~/.config/nexode` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/nexode`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
```json title="Remote config from .well-known/nexode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Placez votre configuration globale Nexode dans `~/.config/nexode/nexode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/nexode/tui.json`.
...
Ajoutez `nexode.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque Nexode démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `NEXODE_CONFIG`.

```bash
export NEXODE_CONFIG=/path/to/my/custom-config.json
nexode run "Hello world"
```
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `NEXODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.nexode`, et devrait suivre la même structure.

```bash
export NEXODE_CONFIG_DIR=/path/to/my/config-directory
nexode run "Hello world"
```

Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.nexode`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`nexode.ai/config.json`**](https://nexode.ai/config.json).

La configuration TUI utilise [**`nexode.ai/tui.json`**](https://nexode.ai/tui.json).
...
```json title="tui.json"
{
  "$schema": "https://nexode.ai/tui.json",
  "scroll_speed": 3,
  "scroll_acceleration": {
    "enabled": true
  },
...
Utilisez `NEXODE_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `nexode.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `nexode serve` et `nexode web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Nexode.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `nexode.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Nexode via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Nexode essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/nexode/agents/` ou `.nexode/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Nexode reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`nexode run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/nexode/commands/` ou `.nexode/commands/`. [En savoir plus ici](/docs/commands).
...
Nexode téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, nexode **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent Nexode avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.nexode/plugins/` ou `~/.config/nexode/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.

```json title="nexode.json"
{
  "$schema": "https://nexode.ai/config.json",
  "plugin": ["nexode-helicone-session", "@my-org/custom-plugin"]
}
```
...
Ceci est utile lorsque vous souhaitez restreindre Nexode à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
