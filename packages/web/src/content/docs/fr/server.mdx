---
title: Serveur
description: Interagissez avec le serveur nexode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `nexode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client nexode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `nexode.local` |
...
Définissez `NEXODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `nexode`, ou définissez `NEXODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `nexode serve` et à `nexode web`.
...
Lorsque vous exécutez `nexode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur nexode pour interagir avec nexode par programmation.
:::

Cette architecture permet à nexode de prendre en charge plusieurs clients et vous permet d'interagir avec nexode par programme.

Vous pouvez exécuter `nexode serve` pour démarrer un serveur autonome. Si vous avez le
nexode TUI en cours d'exécution, `nexode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Nexode [IDE](/docs/ide).
...
Le serveur nexode expose les API suivantes.
