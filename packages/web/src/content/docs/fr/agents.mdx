...
Il existe deux types d'agents dans Nexode ; agents primaires et sous-agents.
...
Nexode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Nexode est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Nexode est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `nexode.json` :
...
- Global : `~/.config/nexode/agents/`
- Par projet : `.nexode/agents/`

```markdown title="~/.config/nexode/agents/review.md"
---
...
Si aucune température n'est spécifiée, Nexode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Nexode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Nexode utilise le format `provider/model-id`. Par exemple, si vous utilisez [Nexode Zen](/docs/zen), vous utiliserez `nexode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="nexode.json" {3-6,9-12}
{
  "$schema": "https://nexode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="nexode.json" {3-5,8-10}
{
  "$schema": "https://nexode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `nexode models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/iamgrootns/Nexode).
:::
...
```markdown title="~/.config/nexode/agents/docs-writer.md"
---
...
```markdown title="~/.config/nexode/agents/security-auditor.md"
---
