---
title: SDK
description: Client JS de type sécurisé pour le serveur nexode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK nexode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler nexode par programme.
...
Créez une instance de nexode :

```javascript
import { createNexode } from "@nexode-ai/sdk"

const { client } = await createNexode()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `nexode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${nexode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de nexode, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@nexode-ai/sdk"
```
