---
title: サーバー
description: HTTP 経由で Nexode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`nexode serve` コマンドは、Nexode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `nexode.local` |
...
HTTP 基本認証でサーバーを保護するには、`NEXODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `nexode` になるか、`NEXODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`nexode serve` と `nexode web` の両方に当てはまります。
...
`nexode` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Nexode サーバーを使用して、プログラムで Nexode と対話します。
:::

`nexode serve` を実行してスタンドアロンサーバーを起動できます。すでに Nexode TUI を実行している場合でも、`nexode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Nexode [IDE](/docs/ide) プラグインによって使用されます。
