...
_Model Bağlam Protokolü_ veya MCP kullanarak nexode'a harici araçlar ekleyebilirsiniz. nexode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [nexode Config](https://nexode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/nexode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
nexode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde nexode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa nexode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `nexode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından nexode, belirteçleri `~/.local/share/nexode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra nexode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi nexode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve nexode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
