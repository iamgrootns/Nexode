...
Ajan becerileri, nexode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
nexode su konumlari tarar:

- Proje konfigurasyonu: `.nexode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/nexode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda nexode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.nexode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/nexode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.nexode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
nexode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`nexode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`nexode.json` icinde):
