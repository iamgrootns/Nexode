---
title: Zen
description: nexode tarafından sağlanan seçilmiş modeller listesi.
---
...
Nexode Zen, nexode ekibi tarafından test edilip doğrulanmış modellerin bir listesidir.

:::note
Nexode Zen şu anda beta aşamasındadır.
:::

Zen, nexode'daki diğer sağlayıcılar gibi çalışır. Nexode Zen'e giriş yapar ve API anahtarınızı alırsınız. Tamamen isteğe bağlıdır ve nexode kullanmak için bunu kullanmanıza gerek yoktur.
...
:::tip
nexode ile iyi çalışan seçkin bir grup model ve sağlayıcıyı test ettik.
:::
...
Nexode Zen, bu modellere erişmenizi sağlayan bir AI ağ geçididir.

---

## Nasıl çalışır

Nexode Zen, nexode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Nexode Zen</a>**'de oturum açın, fatura ayrıntılarınızı ekleyin ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, Nexode Zen'i seçin ve API anahtarınızı yapıştırın.
...
nexode yapılandırmanızdaki [model kimliği](/docs/config/#models) `nexode/<model-id>` biçimini kullanır. Örneğin, GPT 5.2 Codex için yapılandırmanızda `nexode/gpt-5.2-codex` kullanırsınız.
...
- MiniMax M2.5 Free, sınırlı bir süre için Nexode'da ücretsizdir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
- Big Pickle, sınırlı bir süre için Nexode'da ücretsiz olan gizli bir modeldir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
...
Nexode Zen'i şu amaçlarla oluşturduk:
...
4. Başka bir kodlama ajanıyla kullanmanıza izin vererek **kilitlenmeyi önlemek**. Ve her zaman Nexode ile başka bir sağlayıcıyı kullanmanıza izin vermek.
