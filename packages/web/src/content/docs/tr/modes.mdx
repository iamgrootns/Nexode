...
:::caution
Modlar artık nexode yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

nexode'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya nexode yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
nexode iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.nexode/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`nexode.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, nexode modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global nexode yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.nexode/modes/` veya genel modlar için `~/.config/nexode/modes/`'de mod dosyaları oluşturun:
