...
`.nexode/commands/test.md` oluştur:
...
nexode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

nexode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/nexode/commands/`
- Proje başına: `.nexode/commands/`
...
nexode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
