---
title: Eklentiler
description: Nexode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Nexode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Nexode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.nexode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/nexode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/nexode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/nexode/nexode.json`)
2. Project config (`nexode.json`)
3. Global eklenti dizini (`~/.config/nexode/plugins/`)
4. Proje eklenti dizini (`.nexode/plugins/`)
...
Nexode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Nexode SDK istemcisi.
...
Nexode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Nexode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Nexode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Nexode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Nexode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Nexode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://nexode.ai/docs/sdk) bakın.
