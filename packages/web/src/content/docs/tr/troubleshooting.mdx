...
nexode ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\nexode\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `nexode --log-level DEBUG`.

---

## Depolama

nexode, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:
...
nexode Desktop arka planda yerel bir nexode sunucusu (`nexode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `nexode` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/nexode/nexode.jsonc` (veya `~/.config/nexode/nexode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/nexode/nexode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\nexode\nexode.jsonc` yapistirin
...
nexode diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/nexode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\nexode\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), nexode'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. nexode Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/nexode`
- **Linux**: `~/.cache/nexode` dizinini silin (veya `rm -rf ~/.cache/nexode` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\nexode` yapistirin

3. nexode Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

nexode Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`nexode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `NEXODE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `NEXODE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta nexode Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, nexode ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

nexode Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda nexode icin bildirimler etkinse
...
1. nexode Desktop'u kapatin
2. Su dosyalari bulun ve silin (nexode Desktop uygulama veri dizininde yer alirlar):

- `nexode.settings.dat` (desktop varsayilan sunucu URL'si)
- `nexode.global.dat` ve `nexode.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
nexode ile ilgili bir sorun yasiyorsaniz:
...
3. `nexode upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `nexode models` calistirin.
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. nexode, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
2. En guncel provider paketlerini yeniden kurmak icin nexode'u yeniden baslatin

Bu, nexode'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
nexode, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
