---
title: Sunucu
description: nexode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`nexode serve` komutu, nexode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `nexode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `NEXODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `nexode` değeridir; değiştirmek isterseniz `NEXODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `nexode serve` hem de `nexode web` için geçerlidir.
...
`nexode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
nexode ile programatik etkileşim için nexode sunucusunu kullanın.
:::

Bu mimari, nexode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`nexode serve` ile bağımsız bir sunucu başlatabilirsiniz. nexode TUI açıksa, `nexode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum nexode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
nexode sunucusu aşağıdaki API'leri sunar.
