...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. nexode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
nexode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `NEXODE_EXPERIMENTAL_LSP_TOOL=true` (veya `NEXODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca nexode provider kullanılırken veya `NEXODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

nexode başlatırken etkinleştirmek için:
