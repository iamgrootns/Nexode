...
nexode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak nexode kendi `nexode` temasını kullanır.
...
nexode birden fazla yerleşik temayla gelir.
...
- nexode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
nexode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/nexode/themes/*.json` veya `$XDG_CONFIG_HOME/nexode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.nexode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.nexode/themes/*.json`
