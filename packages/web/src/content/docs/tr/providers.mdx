---
title: Sağlayıcılar
description: nexode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

nexode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı nexode yapılandırmanızda yapılandırın.
...
`~/.local/share/nexode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları nexode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Nexode Zen, nexode ekibi tarafından sağlanan modellerin bir listesidir.
nexode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Nexode Zen ile başlamanızı öneririz.
:::
...
nexode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Nexode Go

Nexode Go, nexode ile iyi çalıştığı test edilmiş ve doğrulanmış, nexode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Nexode Go`'yu seçin ve [nexode.ai/auth](https://nexode.ai/zen) adresine gidin.
...
Amazon Bedrock'u nexode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   nexode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `nexode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi nexode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   nexode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Nexode yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in nexode auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
nexode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. nexode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`nexode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [nexode-gitlab-auth](https://www.npmjs.com/package/@gitlab/nexode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`nexode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi nexode'la kullanmak için:
...
Google Vertex AI'yi nexode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları nexode çalıştırılırken ayarlayın.
...
Helicone'un nexode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her nexode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [nexode-helicone-session](https://github.com/H2Shami/nexode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her nexode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
nexode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla nexode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla nexode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini nexode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/nexode)'a bakın.
:::
...
Ollama Cloud'u nexode ile kullanmak için:
...
3. nexode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: nexode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Nexode Zen, nexode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Nexode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Nexode Zen**'i arayın.

   ```txt
   /connect
   ```

3. nexode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   nexode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca nexode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Nexode ile kullanmak için:
...
Modelleri nexode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı nexode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `nexode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları nexode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `nexode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için nexode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, nexode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
