---
title: TUI
description: Nexode terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Nexode, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Nexode'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Nexode TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Nexode'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Nexode'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `nexode.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `NEXODE_TUI_CONFIG` kullanın.
