...
nexode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
nexode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
nexode, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

nexode iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `nexode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse nexode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global nexode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
nexode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Nexode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `nexode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `nexode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/iamgrootns/Nexode).
:::
