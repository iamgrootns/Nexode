...
Aby debugować problemy z nexode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `nexode --log-level DEBUG`.

---

## Przechowywanie danych

nexode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
nexode Desktop uruchamia w tle lokalny serwer nexode (sidecar `nexode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Nexode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/nexode/nexode.jsonc` (lub `~/.config/nexode/nexode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/nexode/nexode.jsonc`
...
nexode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby nexode mógł ją odbudować.

1. Całkowicie zamknij nexode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/nexode`
- **Linux**: usuń `~/.cache/nexode` (lub uruchom `rm -rf ~/.cache/nexode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\nexode`

3. Uruchom ponownie nexode Desktop.

---

### Napraw problemy z połączeniem z serwerem

nexode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `nexode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `NEXODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `NEXODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows nexode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami nexode.

---

### Brak powiadomień

nexode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla nexode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie nexode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji nexode Desktop):

- `nexode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `nexode.global.dat` i `nexode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `nexode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `nexode models`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. nexode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
2. Uruchom ponownie kod nexode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to nexode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
nexode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
