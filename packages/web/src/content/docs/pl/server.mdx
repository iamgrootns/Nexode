---
title: Serwer
description: Interakcja z serwerem nexode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `nexode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient nexode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `nexode.local` |
...
Ustaw `NEXODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `nexode` lub ustaw `NEXODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `nexode serve`, jak i `nexode web`.
...
Kiedy uruchomisz `nexode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera nexode do programowej interakcji z nexode.
:::

Ta architektura umożliwia obsługę wielu klientów przez nexode i programową interakcję z kodem otwartym.

Możesz uruchomić `nexode serve`, aby uruchomić samodzielny serwer. Jeśli masz
nexode TUI działa, `nexode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki nexode [IDE](/docs/ide).
...
Serwer nexode udostępnia następujące interfejsy API.
