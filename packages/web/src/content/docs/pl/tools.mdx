...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Nexode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Nexode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `NEXODE_EXPERIMENTAL_LSP_TOOL=true` (lub `NEXODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Nexode lub gdy zmienna środowiskowa `NEXODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Nexode:
