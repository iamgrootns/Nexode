...
Dzięki nexode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie nexode używa naszego własnego motywu `nexode`.
...
nexode ma kilka wbudowanych motywów.
...
- Chcą, aby Nexode pasował do wyglądu ich terminala
...
nexode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/nexode/themes/*.json` lub `$XDG_CONFIG_HOME/nexode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.nexode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.nexode/themes/*.json`
