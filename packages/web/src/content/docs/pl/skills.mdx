...
Umiejętności agenta pozwalają nexode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
nexode przeszukuje te lokalizacje:

- Project config: `.nexode/skills/<name>/SKILL.md`
- Global config: `~/.config/nexode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu nexode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.nexode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/nexode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.nexode/skills/git-release/SKILL.md` w ten sposób:
...
nexode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `nexode.json`:
...
**Dla agentów wbudowanych** (w `nexode.json`):
