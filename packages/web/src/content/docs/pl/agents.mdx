...
W Nexode obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Nexode zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Nexode ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Nexode ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `nexode.json`:
...
- Globalnie: `~/.config/nexode/agents/`
- W projekcie: `.nexode/agents/`
...
Jeśli nie zostanie skonfigurowana, Nexode używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Nexode, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Nexode w `provider/model-id`. Na przykład, [Nexode Zen](/docs/zen), użyj `nexode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `nexode models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/iamgrootns/Nexode).
:::
