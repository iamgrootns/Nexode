---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć nexode.
---

Wtyczki umożliwiające wyłączenie nexode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania nexode.
...
- `.nexode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/nexode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/nexode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/nexode/nexode.json`)
2. Konfiguracja projektu (`nexode.json`)
3. Globalny katalog wtyczek (`~/.config/nexode/plugins/`)
4. Katalog wtyczek projektu (`.nexode/plugins/`)
...
nexode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z nexode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do nexode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej nexode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj nexode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do nexode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod nexode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://nexode.ai/docs/sdk).
