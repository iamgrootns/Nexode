...
Możesz dodać dodatkowe informacje do nexode za pomocą _Model Context Protocol_, w skrócie MCP. nexode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [nexode Config](https://nexode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/nexode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
nexode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, nexode:
...
Jeśli serwer wymaga uwierzytelnienia, nexode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `nexode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji nexode bezpieczne przechowa tokeny w `~/.local/share/nexode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w nexode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie nexode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć nexode z kontem Sentry.
