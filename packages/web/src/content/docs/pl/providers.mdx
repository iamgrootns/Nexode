---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w nexode.
---

import config from "../../../../config.mjs"
export const console = config.console

nexode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji nexode.
...
w `~/.local/share/nexode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim nexode
config.
...
Nexode Zen to lista modeli dostarczonych przez zespół nexode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z nexode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Nexode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Nexode Zen` i przejdź do [nexode.ai/auth](https://nexode.ai/zen).
...
Działa jak każdy inny dostawca w nexode i jest całkowicie opcjonalny w użyciu.

---

## Nexode Go

Nexode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół nexode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z nexode.

1. Uruchom polecenie `/connect` w TUI, wybierz `Nexode Go` i przejdź do [nexode.ai/auth](https://nexode.ai/zen).
...
Aby używać Amazon Bedrock z nexode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania nexode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `nexode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w nexode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod nexode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji nexode.
...
nexode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować nexode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`nexode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [nexode-gitlab-auth](https://www.npmjs.com/package/@gitlab/nexode-gitlab-auth).

##### Konfiguracja

Customize through `nexode.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem nexode:
...
Aby używać Google Vertex AI z nexode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania nexode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą nexode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [nexode-helicone-session](https://github.com/H2Shami/nexode-helicone-session), aby automatycznie rejestrować każdą konwersację nexode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja nexode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować nexode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować nexode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować nexode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem nexode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/nexode).
:::
...
Aby korzystać z Ollama Cloud z nexode:
...
3. Skopiuj klucz API do użycia w nexode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w nexode musisz pobrać informacje o modelu lokalnie:
...
Nexode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół nexode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Nexode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Nexode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API nexode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji nexode.
...
5. Możesz je także dostosować za pomocą konfiguracji nexode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z nexode:
...
Możesz także dostosować modele za pomocą konfiguracji nexode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z nexode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `nexode.json` w katalogu projektu:
...
Pola `limit` pozwalają nexode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `nexode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację nexode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji nexode.
