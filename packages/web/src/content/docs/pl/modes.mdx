...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji nexode.
...
te lub skonfiguruj własne za pomocą konfiguracji nexode.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.nexode/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `nexode.json`:
...
Jeśli nie ma wpływu na temperaturę, nexode stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna nexode, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.nexode/modes/` dla trybów zapisanych dla projektu lub `~/.config/nexode/modes/` dla trybów globalnych:
