---
title: Providers
description: Using any LLM provider in Nexode.
---

import config from "../../../config.mjs"
export const console = config.console

Nexode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Nexode config.
...
You can customize the providers through the `provider` section in your Nexode
config.
...
Nexode Zen is a list of models provided by the Nexode team that have been
tested and verified to work well with Nexode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Nexode Zen.
:::

1. Run the `/connect` command in the TUI, select `Nexode Zen`, and head to [nexode.ai/auth](https://nexode.ai/zen).
...
It works like any other provider in Nexode and is completely optional to use.

---

## Nexode Go

Nexode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Nexode team that have been
tested and verified to work well with Nexode.

1. Run the `/connect` command in the TUI, select `Nexode Go`, and head to [nexode.ai/auth](https://nexode.ai/zen).
...
To use Amazon Bedrock with Nexode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running nexode:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `nexode.json`:
...
:::info
Using your Claude Pro/Max subscription in Nexode is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for nexode to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your nexode config.
...
Nexode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Nexode
to only use your own GitLab-hosted instance, add the following to your
`nexode.json` file. It is also recommended to disable session sharing.
...
More documentation on [nexode-gitlab-auth](https://www.npmjs.com/package/@gitlab/nexode-gitlab-auth) homepage.
...
To use your GitHub Copilot subscription with nexode:
...
To use Google Vertex AI with Nexode:
...
In the event you see a feature or model from Helicone that isn't configured automatically through nexode, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [nexode-helicone-session](https://github.com/H2Shami/nexode-helicone-session) plugin to automatically log each Nexode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Nexode conversation listed as a separate session.
...
You can configure nexode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure nexode to use local models through LM Studio.
...
You can configure nexode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Nexode. See the [Ollama integration docs](https://docs.ollama.com/integrations/nexode) for details.
:::
...
To use Ollama Cloud with Nexode:
...
3. Copy the API key for use in Nexode.
...
6. **Important**: Before using cloud models in Nexode, you must pull the model information locally:
...
Nexode Zen is a list of tested and verified models provided by the Nexode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Nexode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Nexode Zen**.

   ```txt
   /connect
   ```

3. Enter your Nexode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your nexode config.
...
5. You can also customize them through your nexode config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Nexode:
...
You can also customize models through your nexode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with nexode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `nexode.json` file in your project directory:
...
The `limit` fields allow Nexode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `nexode auth list` to see if the credentials
...
2. For custom providers, check the nexode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your nexode config.
