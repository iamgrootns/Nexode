...
Wenn Nexode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `nexode --log-level DEBUG`.

---

## Speicher

nexode speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
Nexode Desktop startet im Hintergrund einen lokalen Nexode-Server (`nexode-cli`-Sidecar).
...
- Nur macOS: `Nexode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/nexode/nexode.jsonc` (oder `~/.config/nexode/nexode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/nexode/nexode.jsonc`
...
Nexode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Nexode ihn neu aufbauen kann.

1. Beende Nexode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/nexode`
- **Linux**: loesche `~/.cache/nexode` (oder fuehre aus: `rm -rf ~/.cache/nexode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\nexode`

3. Starte Nexode Desktop neu.

---

### Server-Verbindungsprobleme beheben

Nexode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `nexode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `NEXODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `NEXODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Nexode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Nexode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Nexode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Nexode in den OS-Einstellungen aktiviert sind, und
...
1. Beende Nexode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Nexode Desktop):

- `nexode.settings.dat` (desktop default server URL)
- `nexode.global.dat` und `nexode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Nexode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `nexode models` aus.
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. nexode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
2. Starte nexode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt nexode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
nexode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
