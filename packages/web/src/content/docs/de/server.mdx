---
title: Server
description: Nutze den nexode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `nexode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein nexode-Client nutzen kann.
...
Setze `NEXODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `nexode`, kann aber mit `NEXODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `nexode serve` und `nexode web`.
...
Wenn du `nexode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den nexode-Server, um nexode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `nexode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `nexode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Nexode-[IDE](/docs/ide)-Plugins.
...
Der nexode-Server stellt folgende APIs bereit.
