---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Nexode.
---

import config from "../../../../config.mjs"
export const console = config.console

Nexode verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Nexode-Konfiguration.
...
in `~/.local/share/nexode/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem Nexode anpassen
config.
...
Nexode Zen ist eine Liste von Modellen, die vom Nexode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Nexode funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Nexode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Nexode aus und gehen Sie zu [nexode.ai/auth](https://nexode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Nexode und ist völlig optional.

---

## Nexode Go

Nexode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Nexode-Team getestet und verifiziert wurden, dass sie gut mit Nexode funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Nexode Go` und gehen Sie zu [nexode.ai/auth](https://nexode.ai/zen).
...
So verwenden Sie Amazon Bedrock mit Nexode:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Nexode ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `nexode.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Nexode wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Nexode ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Nexode-Konfiguration hinzufügen.
...
um das Token im Nexode-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Nexode verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Nexode
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`nexode.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [nexode-gitlab-auth](https://www.npmjs.com/package/@gitlab/nexode-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `nexode.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Nexode:
...
So verwenden Sie Google Vertex AI mit Nexode:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Nexode ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Nexode konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [nexode-helicone-session](https://github.com/H2Shami/nexode-helicone-session)-Plugin, um jede Nexode-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Nexode-Konversation als separate Sitzung aufgeführt.
...
Sie können Nexode für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Nexode über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Nexode über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Nexode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/nexode).
:::
...
So verwenden Sie Ollama Cloud mit Nexode:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Nexode.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Nexode verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Nexode Zen ist eine Liste getesteter und verifizierter Modelle, die vom Nexode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Nexode Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Nexode Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Nexode API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Nexode-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Nexode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Nexode:
...
Sie können Modelle auch über Ihre Nexode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Nexode verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `nexode.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Nexode erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `nexode auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Nexode-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Nexode-Konfiguration übereinstimmt.
