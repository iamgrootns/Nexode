...
Mit Nexode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Nexode das `nexode`-Theme.
...
Nexode liefert mehrere eingebaute Themes mit.
...
- Nexode optisch ans Terminal anpassen wollen
...
Nexode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/nexode/themes/*.json` oder `$XDG_CONFIG_HOME/nexode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.nexode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.nexode/themes/*.json`
