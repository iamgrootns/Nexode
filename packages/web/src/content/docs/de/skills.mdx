...
Agent Skills erlauben Nexode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Nexode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Nexode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.nexode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/nexode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.nexode/skills/git-release/SKILL.md` zum Beispiel so:
...
Nexode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `nexode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `nexode.json`):
