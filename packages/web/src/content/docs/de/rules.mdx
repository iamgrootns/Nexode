---
title: Regeln
description: Lege eigene Anweisungen fuer nexode fest.
---

Du kannst nexode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in nexode den Befehl `/init` aus.
...
So kann nexode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht nexode sie zu erweitern.
...
nexode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/nexode/AGENTS.md` speichern.
Diese gelten in allen nexode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Nexode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/nexode/AGENTS.md` existiert)
...
Beim Start sucht nexode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/nexode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `nexode.json` oder in `~/.config/nexode/nexode.json` angeben.
...
nexode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit nexode.json

Empfohlen ist das `instructions`-Feld in der `nexode.json`:
...
Du kannst nexode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `nexode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
