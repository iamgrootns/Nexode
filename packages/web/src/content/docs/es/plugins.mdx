---
title: Complementos
description: Escriba sus propios complementos para extender Nexode.
---

Los complementos le permiten extender Nexode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Nexode.
...
- `.nexode/plugins/` - Complementos a nivel de proyecto
- `~/.config/nexode/plugins/` - Complementos globales
...
```json title="nexode.json"
{
  ...
  "plugin": ["nexode-helicone-session", "nexode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/nexode/node_modules/`.
...
1. Configuración global (`~/.config/nexode/nexode.json`)
2. Configuración del proyecto (`nexode.json`)
3. Directorio global de complementos (`~/.config/nexode/plugins/`)
4. Directorio de complementos del proyecto (`.nexode/plugins/`)
...
```json title=".nexode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Nexode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".nexode/plugins/my-plugin.ts"
...
```js title=".nexode/plugins/example.js"
...
- `client`: Un cliente SDK nexode para interactuar con la IA.
...
import type { Plugin } from "@nexode-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar nexode.
...
```js title=".nexode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "nexode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Nexode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que nexode lea archivos `.env`:

```javascript title=".nexode/plugins/env-protection.js"
...
```javascript title=".nexode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a nexode:

```ts title=".nexode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@nexode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que nexode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para nexode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://nexode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".nexode/plugins/compaction.ts"
import type { Plugin } from "@nexode-ai/plugin"
...
```ts title=".nexode/plugins/custom-compaction.ts"
import type { Plugin } from "@nexode-ai/plugin"
