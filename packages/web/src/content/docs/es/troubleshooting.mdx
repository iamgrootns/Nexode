...
Para depurar problemas con Nexode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/nexode/log/`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `nexode --log-level DEBUG`.

---

## Almacenamiento

nexode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/nexode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\nexode`
...
Nexode Desktop ejecuta un servidor Nexode local (el sidecar `nexode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Nexode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/nexode/nexode.jsonc` (o `~/.config/nexode/nexode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/nexode/nexode.jsonc`
...
Nexode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/nexode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\nexode\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Nexode pueda reconstruirlo.

1. Salga de Nexode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/nexode`
- **Linux**: eliminar `~/.cache/nexode` (o ejecutar `rm -rf ~/.cache/nexode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\nexode`

3. Reinicie el escritorio Nexode.

---

### Solucionar problemas de conexión del servidor

Nexode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `nexode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `NEXODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `NEXODE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Nexode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Nexode.

---

### Notificaciones que no se muestran

Nexode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Nexode en la configuración de su sistema operativo, y
...
1. Salga del escritorio Nexode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Nexode):

- `nexode.settings.dat` (URL del servidor predeterminado de escritorio)
- `nexode.global.dat` y `nexode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/iamgrootns/Nexode/issues**](https://github.com/iamgrootns/Nexode/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**nexode.ai/discord**](https://nexode.ai/discord)
...
3. Asegúrese de tener la última versión con `nexode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `nexode models`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. nexode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
2. Reinicie nexode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a nexode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
nexode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
