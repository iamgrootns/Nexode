---
title: Proveedores
description: Usando cualquier proveedor LLM en Nexode.
---

import config from "../../../../config.mjs"
export const console = config.console

Nexode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Nexode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Nexode
configuración.
...
Nexode Zen es una lista de modelos proporcionados por el equipo Nexode que han sido
probado y verificado para funcionar bien con Nexode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Nexode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione nexode y diríjase a [nexode.ai/auth](https://nexode.ai/auth).
...
Funciona como cualquier otro proveedor en Nexode y su uso es completamente opcional.

---

## Nexode Go

Nexode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Nexode que han sido
probado y verificado para funcionar bien con Nexode.

1. Ejecute el comando `/connect` en TUI, seleccione `Nexode Go` y diríjase a [nexode.ai/auth](https://nexode.ai/zen).
...
Para usar Amazon Bedrock con Nexode:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta nexode:
   ...
   Para una configuración persistente o específica del proyecto, utilice `nexode.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Nexode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que nexode funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración nexode.
...
para almacenar el token en el almacenamiento de autenticación nexode.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Nexode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Nexode
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `nexode.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [nexode-gitlab-auth](https://www.npmjs.com/package/@gitlab/nexode-gitlab-auth).

##### Configuración

Personalizar a través de `nexode.json`:
...
Para utilizar su suscripción GitHub Copilot con nexode:
...
Para utilizar Google Vertex AI con Nexode:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta nexode.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de nexode, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [nexode-helicone-session](https://github.com/H2Shami/nexode-helicone-session) para registrar automáticamente cada conversación Nexode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Nexode enumerada como una sesión separada.
...
Puede configurar nexode para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar nexode para usar modelos locales a través de LM Studio.
...
Puedes configurar nexode para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Nexode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/nexode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Nexode:
...
3. Copie la clave API para usarla en Nexode.
...
6. **Importante**: Antes de usar modelos en la nube en Nexode, debe obtener la información del modelo localmente:
...
Nexode Zen es una lista de modelos probados y verificados proporcionada por el equipo Nexode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Nexode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Nexode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Nexode API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración nexode.
...
5. También puedes personalizarlos a través de tu configuración nexode. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Nexode:
...
También puede personalizar modelos a través de su configuración nexode. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con nexode. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `nexode.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Nexode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `nexode auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración nexode y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración nexode.
