...
Las herramientas permiten que LLM realice acciones en su código base. Nexode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Nexode.
...
:::note
Esta herramienta solo está disponible cuando `NEXODE_EXPERIMENTAL_LSP_TOOL=true` (o `NEXODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Nexode o cuando la variable de entorno `NEXODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Nexode:

```bash
NEXODE_ENABLE_EXA=1 nexode
```
