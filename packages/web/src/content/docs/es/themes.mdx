...
Con Nexode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Nexode usa nuestro propio tema `nexode`.
...
Nexode viene con varios temas integrados.
...
- Quiere que Nexode coincida con la apariencia de su terminal
...
Nexode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/nexode/themes/*.json` o `$XDG_CONFIG_HOME/nexode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.nexode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.nexode/themes/*.json`
