...
Las habilidades del agente permiten a Nexode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Nexode busca estas ubicaciones:

- Configuración del proyecto: `.nexode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/nexode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Nexode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.nexode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/nexode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.nexode/skills/git-release/SKILL.md` así:
...
Nexode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `nexode.json`:
...
**Para agentes integrados** (en `nexode.json`):
