---
title: Servidor
description: Interactuar con el servidor nexode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `nexode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente nexode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `nexode.local` |
...
Configure `NEXODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `nexode`, o configure `NEXODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `nexode serve` como a `nexode web`.
...
Cuando ejecuta `nexode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor nexode para interactuar con nexode mediante programación.
:::

Esta arquitectura permite que nexode admita múltiples clientes y le permite interactuar con nexode mediante programación.

Puede ejecutar `nexode serve` para iniciar un servidor independiente. Si tienes el
nexode TUI ejecutándose, `nexode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Nexode [IDE](/docs/ide).
...
El servidor nexode expone las siguientes API.
