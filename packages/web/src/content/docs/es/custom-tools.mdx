...
description: Cree herramientas que LLM pueda llamar en nexode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de nexode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.nexode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/nexode/tools/`.
...
```ts title=".nexode/tools/database.ts" {1}
...
```ts title=".nexode/tools/project.ts" {8}
import { tool } from "@nexode-ai/plugin"
...
```python title=".nexode/tools/add.py"
import sys
...
```ts title=".nexode/tools/python-add.ts" {10}
import { tool } from "@nexode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".nexode/tools/add.py")
