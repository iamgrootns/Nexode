---
title: Server
description: Interact with nexode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `nexode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an nexode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `nexode.local` |
...
Set `NEXODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `nexode`, or set `NEXODE_SERVER_USERNAME` to override it. This applies to both `nexode serve` and `nexode web`.
...
When you run `nexode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the nexode server to interact with nexode programmatically.
:::

This architecture lets nexode support multiple clients and allows you to interact with nexode programmatically.

You can run `nexode serve` to start a standalone server. If you have the
nexode TUI running, `nexode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Nexode [IDE](/docs/ide) plugins.
...
The nexode server exposes the following APIs.
