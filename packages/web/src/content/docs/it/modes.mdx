...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di nexode. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in nexode ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di nexode.
...
nexode include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.nexode/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `nexode.json`:
...
- Globale: `~/.config/nexode/modes/`
- Progetto: `.nexode/modes/`
...
Se non viene specificata alcuna temperatura, nexode usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di nexode sia per quella specifica del progetto.
...
Crea file di modalita in `.nexode/modes/` per modalita specifiche del progetto oppure in `~/.config/nexode/modes/` per modalita globali:
