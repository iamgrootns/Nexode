---
title: Server
description: Interagisci con il server nexode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `nexode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client nexode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `nexode.local` |
...
Imposta `NEXODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `nexode`, oppure imposta `NEXODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `nexode serve` sia per `nexode web`.
...
Quando esegui `nexode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server nexode per interagire con nexode in modo programmatico.
:::

Questa architettura permette a nexode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `nexode serve` per avviare un server standalone. Se la TUI di nexode e gia in esecuzione, `nexode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Nexode.
...
Il server nexode espone le seguenti API.
