...
Puoi aggiungere strumenti esterni a Nexode usando il _Model Context Protocol_, o MCP. Nexode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Nexode Config](https://nexode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/nexode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Nexode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Nexode:
...
Se il server richiede autenticazione, Nexode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `nexode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Nexode memorizzera' i token in modo sicuro in `~/.local/share/nexode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Nexode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Nexode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Nexode al tuo account Sentry.
