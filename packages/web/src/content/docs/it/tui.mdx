---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Nexode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Nexode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Nexode avvia la TUI per la directory corrente.
...
Quando usi la TUI di Nexode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Nexode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `nexode.json`, che configura il comportamento di server/runtime.
...
Usa `NEXODE_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
