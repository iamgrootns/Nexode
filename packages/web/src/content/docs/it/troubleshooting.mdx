...
Per diagnosticare problemi con Nexode, inizia controllando i log e i dati locali che salva su disco.
...
Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `nexode --log-level DEBUG`.

---

## Archiviazione

nexode salva i dati delle sessioni e altri dati dell'applicazione su disco in:
...
Nexode Desktop esegue in background un server locale di Nexode (il sidecar `nexode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Nexode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/nexode/nexode.jsonc`
...
Nexode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Nexode possa ricostruirla.

1. Chiudi completamente Nexode Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/nexode`
- **Linux**: elimina `~/.cache/nexode` (oppure esegui `rm -rf ~/.cache/nexode`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\nexode`

3. Riavvia Nexode Desktop.

---

### Risolvi problemi di connessione al server

Nexode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `nexode.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `NEXODE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `NEXODE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Nexode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Nexode.

---

### Notifiche non visualizzate

Nexode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Nexode nelle impostazioni del sistema operativo, e
...
1. Chiudi Nexode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Nexode Desktop):

- `nexode.settings.dat` (desktop default server URL)
- `nexode.global.dat` e `nexode.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Nexode:
...
3. Assicurati di avere l'ultima versione con `nexode upgrade`
...
Per capire a quali modelli hai accesso, esegui `nexode models`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. nexode installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
2. Riavvia nexode per reinstallare i pacchetti provider piu recenti

Questo forzera nexode a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
nexode rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
