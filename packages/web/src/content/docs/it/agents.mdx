...
In Nexode esistono due tipi di agenti: agenti primari e subagenti.
...
Nexode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Nexode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Nexode include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `nexode.json`:
...
- Globale: `~/.config/nexode/agents/`
- Per progetto: `.nexode/agents/`
...
Se non specifichi una temperature, Nexode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Nexode sia per la config specifica del progetto.
...
L'ID modello nella configurazione Nexode usa il formato `provider/model-id`. Per esempio, se stai usando [Nexode Zen](/docs/zen), useresti `nexode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `nexode models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/iamgrootns/Nexode).
:::
