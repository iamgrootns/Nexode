---
title: Provider
description: Usa qualsiasi provider LLM in Nexode.
---

import config from "../../../../config.mjs"
export const console = config.console

Nexode usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Nexode.
...
Nexode Zen e una lista di modelli forniti dal team Nexode, testati e
verificati per funzionare bene con Nexode. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Nexode Zen.
:::

1. Esegui `/connect` nella TUI, seleziona nexode e vai su [nexode.ai/auth](https://nexode.ai/auth).
...
Funziona come qualsiasi altro provider in Nexode ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Nexode:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui nexode:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `nexode.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Nexode non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché nexode funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di nexode.
...
salvare il token nello storage di auth di nexode.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Nexode usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Nexode
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `nexode.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [nexode-gitlab-auth](https://www.npmjs.com/package/@gitlab/nexode-gitlab-auth).

##### Configurazione

Personalizza tramite `nexode.json`:
...
Per usare il tuo abbonamento GitHub Copilot con nexode:
...
Per usare Google Vertex AI con Nexode:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui nexode.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da nexode, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [nexode-helicone-session](https://github.com/H2Shami/nexode-helicone-session) per loggare automaticamente ogni conversazione di Nexode come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Nexode elencata come sessione separata.
...
Puoi configurare nexode per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare nexode per usare modelli locali tramite LM Studio.
...
Puoi configurare nexode per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Nexode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/nexode) per dettagli.
:::
...
Per usare Ollama Cloud con Nexode:
...
3. Copia la chiave API da usare in Nexode.
...
6. **Importante**: Prima di usare modelli cloud in Nexode, devi scaricare le informazioni del modello localmente:
...
Nexode Zen è una lista di modelli testati e verificati forniti dal team Nexode. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Nexode Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Nexode Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Nexode.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di nexode.
...
5. Puoi anche personalizzarli tramite la configurazione di nexode. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Nexode:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di nexode. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con nexode. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `nexode.json` nella directory del progetto:
...
I campi `limit` permettono a Nexode di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `nexode auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di nexode e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config nexode.
