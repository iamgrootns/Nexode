---
title: Plugin
description: Scrivi plugin per estendere Nexode.
---

I plugin ti permettono di estendere Nexode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Nexode.
...
- `.nexode/plugins/` - plugin a livello progetto
- `~/.config/nexode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/nexode/node_modules/`.
...
1. Config globale (`~/.config/nexode/nexode.json`)
2. Config di progetto (`nexode.json`)
3. Directory plugin globale (`~/.config/nexode/plugins/`)
4. Directory plugin di progetto (`.nexode/plugins/`)
...
Nexode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di nexode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere nexode.
...
:::note
Se usi l'app desktop di Nexode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a nexode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a nexode:
...
L'helper `tool` crea uno strumento personalizzato che nexode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in nexode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://nexode.ai/docs/sdk) per i dettagli.
