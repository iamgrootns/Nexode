...
Le skill degli agenti permettono a Nexode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Nexode cerca in queste posizioni:

- Config di progetto: `.nexode/skills/<name>/SKILL.md`
- Config globale: `~/.config/nexode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Nexode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.nexode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/nexode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.nexode/skills/git-release/SKILL.md` cosi':
...
Nexode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `nexode.json`:
...
**Per agenti integrati** (in `nexode.json`):
