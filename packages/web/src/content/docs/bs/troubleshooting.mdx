...
Da biste otklonili probleme s Nexode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `nexode --log-level DEBUG`.

---

## Pohrana

Nexode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
Nexode Desktop pokreće lokalni Nexode server (`nexode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Nexode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/nexode/nexode.jsonc` (ili `~/.config/nexode/nexode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/nexode/nexode.jsonc`
...
Nexode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Nexode mogao ponovo izgraditi.

1. Potpuno zatvorite Nexode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/nexode`
- **Linux**: obrišite `~/.cache/nexode` (ili pokrenite `rm -rf ~/.cache/nexode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\nexode`

3. Ponovo pokrenite Nexode Desktop.

---

### Rješavanje problema sa vezom na serveru

Nexode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `nexode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `NEXODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `NEXODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Nexode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Nexode karakteristikama.

---

### Obavještenja se ne prikazuju

Nexode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Nexode u postavkama vašeg OS-a, i
...
1. Zatvorite Nexode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Nexode Desktop aplikacije):

- `nexode.settings.dat` (URL zadanog servera za desktop)
- `nexode.global.dat` i `nexode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `nexode models`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Nexode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite Nexode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Nexode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Nexode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
