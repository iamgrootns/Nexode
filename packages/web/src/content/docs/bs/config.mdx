---
title: Konfiguracija
description: Korištenje Nexode JSON konfiguracije.
---

Možete konfigurirati Nexode koristeći JSON konfiguracijski fajl.

---

## Format

Nexode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/nexode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/nexode/nexode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`NEXODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`nexode.json` u projektu) - postavke specifične za projekat
5. **`.nexode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`NEXODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.nexode` i `~/.config/nexode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/nexode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Nexode konfiguraciju u `~/.config/nexode/nexode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/nexode/tui.json`.
...
Dodajte `nexode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Nexode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `NEXODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `NEXODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.nexode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.nexode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`nexode.ai/config.json`**](https://nexode.ai/config.json).

TUI konfiguracija koristi [**`nexode.ai/tui.json`**](https://nexode.ai/tui.json).
...
Koristite `NEXODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `nexode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `nexode serve` i `nexode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Nexode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `nexode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Nexode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Nexode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/nexode/agents/` ili `.nexode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Nexode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`nexode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/nexode/commands/` ili `.nexode/commands/`. [Saznajte više ovdje](/docs/commands).
...
Nexode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Nexode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Nexode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.nexode/plugins/` ili `~/.config/nexode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Nexode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
