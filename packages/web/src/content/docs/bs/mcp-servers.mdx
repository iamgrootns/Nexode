...
Možete dodati vanjske alate u Nexode koristeći _Model Context Protocol_, ili MCP. Nexode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Nexode Config](https://nexode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/nexode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Nexode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Nexode će:
...
Ako server zahtijeva autentifikaciju, Nexode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `nexode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Nexode sigurno čuva tokene u `~/.local/share/nexode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Nexode, zajedno s ugrađenim alatima. Možete njima upravljati kroz Nexode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Nexode sa vašim Sentry nalogom.
