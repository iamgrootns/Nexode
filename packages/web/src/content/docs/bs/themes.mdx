...
U Nexode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Nexode koristi nasu `nexode` temu.
...
Nexode dolazi sa vise ugradenih tema.
...
- Zele da Nexode odgovara izgledu njihovog terminala
...
Nexode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/nexode/themes/*.json` ili `$XDG_CONFIG_HOME/nexode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.nexode/themes/*.json`
4. **Trenutni radni direktorij** - `./.nexode/themes/*.json`
