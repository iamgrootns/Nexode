---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Nexode.
---

Dodaci vam omogućavaju da proširite Nexode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Nexode.
...
- `.nexode/plugins/` - Dodaci na nivou projekta
- `~/.config/nexode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/nexode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/nexode/nexode.json`)
2. Konfiguracija projekta (`nexode.json`)
3. Globalni direktorij dodataka (`~/.config/nexode/plugins/`)
4. Direktorij dodataka projekta (`.nexode/plugins/`)
...
Nexode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Nexode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Nexode.
...
Spriječite nexode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u nexode:
...
Pomoćnik `tool` kreira prilagođeni alat koji nexode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za nexode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://nexode.ai/docs/sdk) za detalje.
