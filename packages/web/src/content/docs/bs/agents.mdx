...
Postoje dvije vrste agenata u Nexode: primarni agenti i podagenti.
...
Nexode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Nexode dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Nexode dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `nexode.json`:
...
- Globalno: `~/.config/nexode/agents/`
- Po projektu: `.nexode/agents/`
...
Ako temperatura nije navedena, Nexode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Nexode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Nexode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Nexode Zen](/docs/zen), koristili biste `nexode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `nexode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/iamgrootns/Nexode).
:::
