...
Agent skills omogucavaju Nexode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Nexode pretrazuje ove lokacije:

- Konfiguracija projekta: `.nexode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/nexode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Nexode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.nexode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/nexode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.nexode/skills/git-release/SKILL.md` ovako:
...
Nexode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `nexode.json`:
...
**Za ugradene agente** (u `nexode.json`):
