...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Nexode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Nexode.
...
:::note
Ovaj alat je dostupan samo kada je `NEXODE_EXPERIMENTAL_LSP_TOOL=true` (ili `NEXODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Nexode provajdera ili kada je varijabla `NEXODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Nexode:
