---
title: Server
description: Komunicirajte s nexode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `nexode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji nexode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `nexode.local` |
...
Postavite `NEXODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `nexode`, ili postavite `NEXODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `nexode serve` i za `nexode web`.
...
Kada pokrenete `nexode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite nexode server za programsku interakciju sa nexode-om.
:::

Ova arhitektura omogucava nexode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `nexode serve` da startate standalone server. Ako je nexode TUI vec pokrenut, `nexode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Nexode [IDE](/docs/ide) pluginovi.
...
nexode server izlaže sljedece API-je.
