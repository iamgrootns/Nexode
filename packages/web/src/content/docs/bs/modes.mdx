...
ove ili konfigurirajte svoje putem nexode config.
...
nexode dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.nexode/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `nexode.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/nexode/modes/`
- Projekat: `.nexode/modes/`
...
Ako temperatura nije navedena, nexode koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.nexode/modes/` za specifične načine rada ili `~/.config/nexode/modes/` za globalne načine:
