---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Nexode.
---

import config from "../../../../config.mjs"
export const console = config.console

Nexode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Nexode konfiguraciji.
...
u `~/.local/share/nexode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Nexode
config.
...
Nexode Zen je lista modela koje je obezbedio Nexode tim koji su bili
testirano i potvrđeno da dobro radi sa Nexode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Nexode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite nexode i idite na [nexode.ai/auth](https://nexode.ai/auth).
...
Radi kao i svaki drugi provajder u Nexode i potpuno je opcionalan za korištenje.

---

## Nexode Go

Nexode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Nexode tim i koji su testirani i verificirani da dobro rade s Nexode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Nexode Go` i idite na [nexode.ai/auth](https://nexode.ai/zen).
...
Da biste koristili Amazon Bedrock s Nexode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete nexode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `nexode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Nexode.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi nexode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju nexode konfiguraciju.
...
da pohrani token u nexode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Nexode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Nexode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`nexode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [nexode-gitlab-auth](https://www.npmjs.com/package/@gitlab/nexode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `nexode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s nexode:
...
Za korištenje Google Vertex AI s Nexode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće nexode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem nexodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [nexode-helicone-session](https://github.com/H2Shami/nexode-helicone-session) da automatski prijavite svaki Nexode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Nexode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati nexode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati nexode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati nexode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Nexode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/nexode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Nexode:
...
3. Kopirajte API ključ za korištenje u Nexode.
...
6. **Važno**: Prije upotrebe modela oblaka u Nexode, morate lokalno povući informacije o modelu:
...
Nexode Zen je lista testiranih i verifikovanih modela koju je obezbedio Nexode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Nexode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Nexode Zen**.

```txt
   /connect
```

3. Unesite svoj Nexode API ključ.
...
Također možete dodati dodatne modele putem vaše nexode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše nexode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Nexodeom:
...
Također možete prilagoditi modele kroz svoju nexode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s nexode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `nexode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Nexode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `nexode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Nexode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj nexode konfiguraciji.
