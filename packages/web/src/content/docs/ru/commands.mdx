...
Создайте `.nexode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию nexode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем nexode [config](/docs/config):
...
- Глобальный: `~/.config/nexode/commands/`
- Для каждого проекта: `.nexode/commands/`
...
nexode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
