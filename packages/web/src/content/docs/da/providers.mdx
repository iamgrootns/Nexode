---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Nexode.
---

import config from "../../../../config.mjs"
export const console = config.console

Nexode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Nexode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/nexode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Nexode-konfigurationen.
...
Nexode Zen er en liste over modeller leveret af Nexode-teamet, som er blevet testet og verificeret til at fungere godt med Nexode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Nexode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg nexode og gå til [nexode.ai/auth](https://nexode.ai/auth).
...
Det fungerer som alle andre udbydere i Nexode og er helt valgfrit at bruge.

---

## Nexode Go

Nexode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Nexode-teamet, som er testet og verificeret til at fungere godt med Nexode.

1. Kør kommandoen `/connect` i TUI, vælg `Nexode Go`, og gå til [nexode.ai/auth](https://nexode.ai/zen).
...
Sådan bruger du Amazon Bedrock med Nexode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører nexode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `nexode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Nexode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at nexode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem nexode-konfigurationen.
...
gemme tokenet i nexode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Nexode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Nexode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`nexode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [nexode-gitlab-auth](https://www.npmjs.com/package/@gitlab/nexode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `nexode.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Nexode:
...
Sådan bruger du Google Vertex AI med Nexode:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem nexode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [nexode-helicone-session](https://github.com/H2Shami/nexode-helicone-session) plugin for automatisk at logge hver Nexode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Nexode-samtale opført som en separat session.
...
Du kan konfigurere nexode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere nexode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere nexode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Nexode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/nexode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Nexode:
...
3. Kopier API-nøglen til brug i Nexode.
...
6. **Vigtigt**: Før du bruger skymodeller i Nexode, skal du hente modelinformationen lokalt:
...
Nexode Zen er en liste over testede og verificerede modeller leveret af Nexode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Nexode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Nexode Zen**.

   ```txt
   /connect
   ```

3. Indtast Nexode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem nexode-konfigurationen.
...
5. Du kan også tilpasse dem gennem nexode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Nexode:
...
Du kan også tilpasse modeller gennem nexode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med nexode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `nexode.json`-filen i projektkataloget:
...
`limit`-felterne lader Nexode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `nexode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek nexode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i nexode-konfigurationen.
