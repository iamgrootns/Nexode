...
Med Nexode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Nexode vårt eget `nexode`-tema.
...
Nexode kommer med flere innebygde temaer.
...
- Vil at Nexode skal matche terminalens utseende
...
Nexode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/nexode/themes/*.json` eller `$XDG_CONFIG_HOME/nexode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.nexode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.nexode/themes/*.json`
