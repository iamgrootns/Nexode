---
title: Plugins
description: Skriv dine egne plugins for at udvide Nexode.
---

Plugins giver dig mulighed for at udvide Nexode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Nexodes standardadfærd.
...
- `.nexode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/nexode/node_modules/`.
...
1. Global konfiguration (`~/.config/nexode/nexode.json`)
2. Projektkonfiguration (`nexode.json`)
3. Global plugin-mappe (`~/.config/nexode/plugins/`)
4. Projekt plugin bibliotek (`.nexode/plugins/`)
...
Nexode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En nexode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide nexode.
...
:::note
Hvis du bruger Nexode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå nexode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til nexode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som nexode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for nexode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://nexode.ai/docs/sdk) for detaljer.
