...
Her er alle de innebygde verktøyene som er tilgængelige i Nexode.
...
:::note
Dette verktøyet er bare tilgængelig når `NEXODE_EXPERIMENTAL_LSP_TOOL=true` (eller `NEXODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Nexode-leverandøren eller når miljøvariabelen `NEXODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
