...
Der er to typer agenter i Nexode; primære agenter og subagenter.
...
Nexode leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Nexode leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

Nexode leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `nexode.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger Nexode modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Nexode-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Nexode-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [Nexode Zen](/docs/zen), vil du bruge `nexode/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `nexode models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/iamgrootns/Nexode).
:::
