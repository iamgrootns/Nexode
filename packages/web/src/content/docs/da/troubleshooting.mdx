...
For at fejlfinde problemer med Nexode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `nexode --log-level DEBUG`.

---

## Lagring

nexode gemmer sessionsdata og andre applikationsdata på disken på:
...
Nexode Desktop kører en lokal Nexode-server (`nexode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Nexode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/nexode/nexode.jsonc` (eller `~/.config/nexode/nexode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/nexode/nexode.jsonc`
...
Nexode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Nexode kan genopbygge den.

1. Afslut Nexode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/nexode`
- **Linux**: slet `~/.cache/nexode` (eller kør `rm -rf ~/.cache/nexode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\nexode`

3. Start Nexode Desktop på ny.

---

### Løs problemer med serverforbindelse

Nexode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `nexode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `NEXODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `NEXODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Nexode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Nexode.

---

### Meddelelser vises ikke

Nexode Desktop viser kun systemvarsler når:

- varsler er aktiveret for Nexode i dine OS-indstillinger, og
...
1. Afslut Nexode Desktop.
2. Find og slet disse filer (de findes i Nexode Desktop-appens datamappe):

- `nexode.settings.dat` (skrivebordsstandardserver URL)
- `nexode.global.dat` og `nexode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Nexode:
...
For at finde ud af hvilke modeller du har adgang til, kør `nexode models`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. nexode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
2. Start nexode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge nexode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
nexode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
