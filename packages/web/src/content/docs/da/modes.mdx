...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i nexode-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i nexode giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem nexode-konfigurationen.
...
nexode leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.nexode/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `nexode.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger nexode modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale nexode-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.nexode/modes/` for projektspecifikke tilstande eller `~/.config/nexode/modes/` for globale tilstande:
