---
title: Server
description: Interager med nexode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `nexode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en nexode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `nexode.local` |
...
Indstil `NEXODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `nexode`, eller indstil `NEXODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `nexode serve` og `nexode web`.
...
Når du kører `nexode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug nexode-serveren til at interagere med nexode programmatisk.
:::

Denne arkitektur lader nexode understøtte flere klienter og lader dig interagere med nexode programmatisk.

Du kan køre `nexode serve` for at starte en selvstændig server. Hvis du har
nexode TUI kørende, vil `nexode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Nexode [IDE](/docs/ide) plugins.
...
Nexode-serveren viser følgende API'er.
