...
Create `.nexode/commands/test.md`:
...
You can add custom commands through the Nexode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Nexode [config](/docs/config):
...
nexode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
