---
title: TUI
description: Using the Nexode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Nexode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Nexode starts the TUI for the current directory.
...
When using the Nexode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Nexode. Allows you to select from available providers and add their API keys.
...
This is separate from `nexode.json`, which configures server/runtime behavior.
...
Use `NEXODE_TUI_CONFIG` to load a custom TUI config path.
