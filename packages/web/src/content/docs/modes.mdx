...
:::caution
Modes are now configured through the `agent` option in the nexode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in nexode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the nexode config.
...
nexode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.nexode/plans/*.md` to detail the plan itself
...
Configure modes in your `nexode.json` config file:
...
both the global nexode config and the project specific config.
...
Create mode files in `.nexode/modes/` for project-specific modes or `~/.config/nexode/modes/` for global modes:
