---
title: المزوّدون
description: استخدام أي موفّر LLM في Nexode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Nexode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Nexode.
...
في `~/.local/share/nexode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Nexode.
...
Nexode Zen هي قائمة نماذج يوفّرها فريق Nexode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Nexode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Nexode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر nexode، ثم انتقل إلى [nexode.ai/auth](https://nexode.ai/auth).
...
يعمل مثل أي مزوّد آخر في Nexode واستخدامه اختياري بالكامل.

---

## Nexode Go

Nexode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Nexode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Nexode.

1. شغّل الأمر `/connect` في TUI، واختر `Nexode Go`، ثم انتقل إلى [nexode.ai/auth](https://nexode.ai/zen).
...
لاستخدام Amazon Bedrock مع Nexode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل nexode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `nexode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Nexode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل nexode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات nexode.
...
تخزين الرمز في مساحة تخزين مصادقة nexode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Nexode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Nexode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`nexode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [nexode-gitlab-auth](https://www.npmjs.com/package/@gitlab/nexode-gitlab-auth).

##### التهيئة

خصّص عبر `nexode.json`:
...
لاستخدام اشتراك GitHub Copilot مع nexode:
...
لاستخدام Google Vertex AI مع Nexode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر nexode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [nexode-helicone-session](https://github.com/H2Shami/nexode-helicone-session) لتسجيل كل محادثة Nexode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Nexode مدرجة كجلسة مستقلة.
...
يمكنك ضبط nexode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط nexode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط nexode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Nexode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/nexode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Nexode:
...
3. انسخ مفتاح API لاستخدامه في Nexode.
...
6. **مهم**: قبل استخدام نماذج السحابة في Nexode، يجب سحب معلومات النموذج محليا:
...
Nexode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Nexode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Nexode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Nexode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Nexode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات nexode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات nexode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Nexode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات nexode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع nexode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `nexode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Nexode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `nexode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات nexode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات nexode.
