...
تتيح مهارات الوكيل لـ Nexode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Nexode في هذه المواقع:

- إعدادات المشروع: `.nexode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/nexode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Nexode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.nexode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/nexode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.nexode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Nexode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `nexode.json`:
...
**للوكلاء المدمجين** (في `nexode.json`):
