...
مع Nexode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Nexode سمتنا `nexode`.
...
- يريدون أن يطابق Nexode مظهر terminal لديهم
...
يدعم Nexode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/nexode/themes/*.json` أو `$XDG_CONFIG_HOME/nexode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.nexode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.nexode/themes/*.json`
