...
أنشئ الملف `.nexode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Nexode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Nexode:
...
- على مستوى النظام: `~/.config/nexode/commands/`
- لكل مشروع: `.nexode/commands/`
...
يتضمن nexode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
