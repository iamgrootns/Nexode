...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("nexode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("nexode_lib=debug,nexode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("nexode_lib=info,nexode_desktop=info,sidecar=info")
        }
    });
