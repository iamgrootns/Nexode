...
Replace runtime Bun shell template-tag usage in `packages/nexode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/nexode/src` is removed except:
