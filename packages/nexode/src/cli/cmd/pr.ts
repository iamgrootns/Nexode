...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run nexode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for nexode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found nexode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`nexode import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting nexode...")
        UI.println()

        // Launch nexode TUI with session ID if available
        const { spawn } = await import("child_process")
        const nexodeArgs = sessionId ? ["-s", sessionId] : []
        const nexodeProcess = spawn("nexode", nexodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          nexodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`nexode exited with code ${code}`))
          })
          nexodeProcess.on("error", reject)
        })
      },
    })
  },
