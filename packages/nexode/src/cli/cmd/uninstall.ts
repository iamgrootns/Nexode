...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall nexode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Nexode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g nexode-ai",
      pnpm: "pnpm uninstall -g nexode-ai",
      bun: "bun remove -g nexode-ai",
      yarn: "yarn global remove nexode-ai",
      brew: "brew uninstall nexode",
      choco: "choco uninstall nexode",
      scoop: "scoop uninstall nexode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall nexode -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using Nexode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# nexode") || content.includes(".nexode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".nexode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".nexode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".nexode"))
    ) {
      continue
    }
