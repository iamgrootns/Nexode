...
export const ServeCommand = cmd({
  ...
  describe: "starts a headless nexode server",
  handler: async (args) => {
    if (!Flag.NEXODE_SERVER_PASSWORD) {
      console.log("Warning: NEXODE_SERVER_PASSWORD is not set; server is unsecured.")
    }
    const opts = await resolveNetworkOptions(args)
    const server = Server.listen(opts)
    console.log(`nexode server listening on http://${server.hostname}:${server.port}`)
