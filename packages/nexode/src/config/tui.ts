...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.NEXODE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.NEXODE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.NEXODE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".nexode") && dir !== Flag.NEXODE_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old nexode.json shape) still get their settings applied.
    const normalized = (() => {
