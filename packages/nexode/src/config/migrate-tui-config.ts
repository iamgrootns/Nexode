...
const TUI_SCHEMA_URL = "https://nexode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from nexode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const nexode = await nexodeFiles(input)
  for (const file of nexode) {
...
async function nexodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.NEXODE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("nexode", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "nexode")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "nexode"))
  }
  if (Flag.NEXODE_CONFIG) files.push(Flag.NEXODE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "nexode"))
