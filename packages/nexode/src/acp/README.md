...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for nexode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal nexode sessions
...
nexode acp --cwd /path/to/project
```
...
```bash
NEXODE_ENABLE_QUESTION_TOOL=1 nexode acp
```
...
- **Terminal Integration**: Full terminal support via nexode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | nexode acp
```
...
ACP sessions map cleanly to nexode's internal session model:
