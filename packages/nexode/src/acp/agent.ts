...
import type { AssistantMessage, Event, NexodeClient, SessionMessageResponse, ToolPart } from "@nexode-ai/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: NexodeClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `nexode auth login` in the terminal",
        name: "Login with nexode",
        id: "nexode-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Nexode stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const nexodeProvider = providers.find((p) => p.id === "nexode")
    if (nexodeProvider) {
      if (nexodeProvider.models["big-pickle"]) {
        return { providerID: "nexode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(nexodeProvider.models))
      if (best) {
        return {
