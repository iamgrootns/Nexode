...
const managedConfigDir = process.env.NEXODE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "nexode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "nexode.json") {
...
test("ignores legacy tui keys in nexode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://nexode.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "nexode.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "nexode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .nexode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nexodeDir = path.join(dir, ".nexode")
      await fs.mkdir(nexodeDir, { recursive: true })
      const agentDir = path.join(nexodeDir, "agent")
...
test("loads agents from .nexode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nexodeDir = path.join(dir, ".nexode")
      await fs.mkdir(nexodeDir, { recursive: true })

      const agentsDir = path.join(nexodeDir, "agents")
...
test("loads commands from .nexode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nexodeDir = path.join(dir, ".nexode")
      await fs.mkdir(nexodeDir, { recursive: true })

      const commandDir = path.join(nexodeDir, "command")
...
test("loads commands from .nexode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nexodeDir = path.join(dir, ".nexode")
      await fs.mkdir(nexodeDir, { recursive: true })

      const commandsDir = path.join(nexodeDir, "commands")
...
test("does not try to install dependencies in read-only NEXODE_CONFIG_DIR", async () => {
  ...
  const prev = process.env.NEXODE_CONFIG_DIR
  process.env.NEXODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.NEXODE_CONFIG_DIR
    else process.env.NEXODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable NEXODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "nexode.json"),
        JSON.stringify({ $schema: "https://nexode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "nexode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .nexode config
      const projectDir = path.join(dir, "project")
      const nexodeDir = path.join(projectDir, ".nexode")
      await fs.mkdir(nexodeDir, { recursive: true })
      ...
      // Local .nexode config with different plugins
      await Filesystem.write(
        path.join(nexodeDir, "nexode.json"),
        JSON.stringify({
          $schema: "https://nexode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .nexode config with some overlapping plugins
      await Filesystem.write(
        path.join(nexodeDir, "nexode.json"),
        JSON.stringify({
          $schema: "https://nexode.ai/config.json",
...
// Note: preload.ts sets NEXODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .nexode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .nexode directory config enables it
      const nexodeDir = path.join(dir, ".nexode")
      await fs.mkdir(nexodeDir, { recursive: true })
      await Filesystem.write(
        path.join(nexodeDir, "nexode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/nexode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/nexode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-nexode@2.4.3")).toBe("oh-my-nexode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@nexode/plugin@2.0.0")).toBe("@nexode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-nexode@2.4.3", "file:///project/.nexode/plugin/oh-my-nexode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.nexode/plugin/oh-my-nexode.js")
  })
  ...
  test("local plugin directory overrides global nexode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const nexodeDir = path.join(projectDir, ".nexode")
        const pluginDir = path.join(nexodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("NEXODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["NEXODE_DISABLE_PROJECT_CONFIG"]
    process.env["NEXODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["NEXODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["NEXODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .nexode/ directories when flag is set", async () => {
    const originalEnv = process.env["NEXODE_DISABLE_PROJECT_CONFIG"]
    process.env["NEXODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .nexode directory with a command
          const nexodeDir = path.join(dir, ".nexode", "command")
          await fs.mkdir(nexodeDir, { recursive: true })
          await Filesystem.write(path.join(nexodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .nexode should NOT be in directories list
          const hasProjectNexode = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectNexode).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["NEXODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NEXODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["NEXODE_CONFIG_DIR"]
      process.env["NEXODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["NEXODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["NEXODE_CONFIG_DIR"]
      } else {
        process.env["NEXODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("NEXODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["NEXODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NEXODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["NEXODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["NEXODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from NEXODE_CONFIG_DIR, not project
...
describe("NEXODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in NEXODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["NEXODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["NEXODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://nexode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["NEXODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["NEXODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in NEXODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["NEXODE_CONFIG_CONTENT"]
