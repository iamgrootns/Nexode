...
**Only** add if it's specifically about the Desktop application or `nexode web` view. **Do not** add for terminal, TUI, or general nexode issues.
...
**Only** add if the issue mentions "zen" or "nexode zen" or "nexode black".

If the issue doesn't have "zen" or "nexode black" in it then don't add zen label

#### core

Use for core server issues in `packages/nexode/`, excluding `packages/nexode/src/cli/cmd/tui/`.
...
TUI (`packages/nexode/src/cli/cmd/tui/...`):
...
Core (`packages/nexode/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for nexode server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
