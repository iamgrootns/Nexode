...
async function fix(pr: PR, files: string[]) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with nexode...`)
  const prompt = [
  ...
  try {
    await $`nexode run -m nexode/gpt-5.3-codex ${prompt}`
  } catch (err) {
    console.log(`  nexode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with nexode")
  return true
}
